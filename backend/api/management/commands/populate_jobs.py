...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if created:
            ...
            Profile.objects.create(user=client_user, role=Profile.ROLE_CLIENT)
            self.stdout.write(self.style.SUCCESS(f'Created client user: {client_user.username}'))
        else:
            self.stdout.write(f'Client user already exists: {client_user.username}')            # Update profile to client role if needed
